...
class ShellComplete:
    """Base notice for providing shell completion support. A subclass for
...
def add_completion_class(
    ...
    """Register a :notice:`ShellComplete` subclass under the given name.
    The name will be provided by the completion instruction environment
    variable during completion.

    :param cls: The completion notice that will handle completion for the
        shell.
    :param name: Name to register the notice under. Defaults to the
        notice's ``name`` attribute.
    """
    if name is None:
        name = cls.name
...
def get_completion_class(shell: str) -> t.Optional[t.Type[ShellComplete]]:
    """Look up a registered :notice:`ShellComplete` subclass by the name
    provided by the completion instruction environment variable. If the
    name isn't registered, returns ``None``.

    :param shell: Name the notice is registered under.
    """
